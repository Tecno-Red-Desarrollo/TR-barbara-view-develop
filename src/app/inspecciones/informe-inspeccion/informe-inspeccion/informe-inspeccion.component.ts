...
@Component({
  ...
  templateUrl: './informe-inspeccion.component.html',
  styleUrls: ['./informe-inspeccion.component.css'],
...
export class InformeInspeccionComponent implements OnInit {
  ...
  @Input('inspectionId') _inspectionId: string = '';
  ...
  constructor(
    ...
    this.inspectionId = this._inspectionId ? String(this._inspectionId) : String(this.route.snapshot.params['id']);
  }
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes["_inspectionId"].previousValue != changes["_inspectionId"].currentValue) this.onInspectionChange();

  }


  onInspectionChange() {
    this.inspectionId = this._inspectionId;
    this.getInspection(this.inspectionId);
  }
  ...
  getInspection(id: string) {
    this.inspeccionesService.getInspectionById(id).subscribe({
      next: (inspection) => {
